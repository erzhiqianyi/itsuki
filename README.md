...
- **個人の記録**：近況、自己紹介、読書・映画・ゲームのアーカイブ、更新履歴。
...
| `npm run photos:studio` | 写真スタジオを起動。ブラウザで写真を R2 にアップロードし、アルバムを作成 |
| `npm run photos:add` | 同じ処理をコマンドラインから実行 |
| `npm run photos:lock` | 既存のアルバムにパスワードを設定（[写真の取り込み](docs/photo-import.md#6-非公開アルバムパスワード保護)） |
...
詳しい運用手順は以下のドキュメントを参照してください。「写真の取り込み」は日本語で書かれていますが、他の補足資料は現在、中国語で記述されています。

- [コンテンツ管理](docs/content-maintenance.md)：各コンテンツの保存先と編集方法。
- [写真の取り込み](docs/photo-import.md)：R2 の準備、写真スタジオの使い方、アルバムの作成。
